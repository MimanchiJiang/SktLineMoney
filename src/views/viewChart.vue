...
export default class Statistics extends Vue {
  ...
  mounted() {
    ...
    console.log(this.tagGroupedList)
  }
  ...
  get tagGroupedList() {
    ...
    // 先筛选出记录中的所有tag并去重
    const allTags = newList.map((r) => r.tags).filter((t) => t !== null).reduce((sum, current) => [...sum, ...current], []);
    const uniqTags = _.uniqBy(allTags, 'id');

    // 再以tag为基准，对收支记录进行分组；即我们需要知道：某个tag，它对应有哪些收支记录
    const groupedList = uniqTags.map((tag) => {
      const tagRecords = newList.filter((r) => r.tags.find((t) => t.id === tag.id));
